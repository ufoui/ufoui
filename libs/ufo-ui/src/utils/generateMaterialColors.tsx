...
import { PartialThemeSchemes, ThemeSchemeKeys, ThemeSchemes } from '@ufoui/types';
...
export function generateMaterialColors(
    ...
    Object.keys(extraSemanticColors).forEach(colorName => {
        const lightColor = customSchemes.light?.[colorName as keyof typeof extraSemanticColors];
        if (lightColor) {
            customColorsList.push({ value: argbFromHex(lightColor), name: colorName, blend: true });
        } else {
            customColorsList.push({
                value: argbFromHex(extraSemanticColors[colorName as keyof typeof extraSemanticColors]),
                name: colorName,
                blend: true,
            });
        }
    ...
    ['light', 'dark'].forEach(scheme => {
        ...
        ThemeSchemeKeys.forEach(key => {
            ...
            } else if (typeof theme.schemes[mdScheme][key as keyof typeof theme.schemes.light] === 'number') {
        ...
        ['light', 'dark'].forEach(scheme => {
            ...
            Object.keys(colorMap).forEach(key => {
                schemes[scheme][colorMap[key]] =
                    customSchemes[scheme]?.[colorMap[key]] ?? hexFromArgb(item[mdScheme][key as keyof ColorGroup]);
            });
        });
    });
    ...
    ['primary', 'secondary', 'tertiary', 'error'].forEach(colorName => {
        ...
        ['light', 'dark'].forEach(scheme => {
            schemes[scheme][`${colorName}Fixed`] = customSchemes[scheme]?.[`${colorName}Fixed`]
                ? customSchemes[scheme][`${colorName}Fixed`]
                : hexFromArgb(theme.palettes[colorName as keyof typeof theme.palettes].tone(90));
            schemes[scheme][`${colorName}FixedDim`] = customSchemes[scheme]?.[`${colorName}FixedDim`]
                ? customSchemes[scheme][`${colorName}FixedDim`]
                : hexFromArgb(theme.palettes[colorName as keyof typeof theme.palettes].tone(80));
            schemes[scheme][`on${pascalColorName}Fixed`] = customSchemes[scheme]?.[`on${pascalColorName}Fixed`]
                ? customSchemes[scheme][`on${pascalColorName}Fixed`]
                : hexFromArgb(theme.palettes[colorName as keyof typeof theme.palettes].tone(10));
            schemes[scheme][`on${pascalColorName}FixedVariant`] = customSchemes[scheme]?.[
                `on${pascalColorName}FixedVariant`
            ]
                ? customSchemes[scheme][`on${pascalColorName}FixedVariant`]
                : hexFromArgb(theme.palettes[colorName as keyof typeof theme.palettes].tone(30));
        });
    });
    ...
    schemes.light.surface = customSchemes.light?.surface ?? hexFromArgb(theme.palettes.neutral.tone(99));
    schemes.light.surfaceDim = customSchemes.light?.surfaceDim ?? hexFromArgb(theme.palettes.neutral.tone(87));
    schemes.light.surfaceBright = customSchemes.light?.surfaceBright ?? hexFromArgb(theme.palettes.neutral.tone(99));
    schemes.light.surfaceContainerLowest =
        customSchemes.light?.surfaceContainerLowest ?? hexFromArgb(theme.palettes.neutral.tone(100));
    schemes.light.surfaceContainerLow =
        customSchemes.light?.surfaceContainerLow ?? hexFromArgb(theme.palettes.neutral.tone(96));
    schemes.light.surfaceContainer =
        customSchemes.light?.surfaceContainer ?? hexFromArgb(theme.palettes.neutral.tone(94));
    schemes.light.surfaceContainerHigh =
        customSchemes.light?.surfaceContainerHigh ?? hexFromArgb(theme.palettes.neutral.tone(92));
    schemes.light.surfaceContainerHighest =
        customSchemes.light?.surfaceContainerHighest ?? hexFromArgb(theme.palettes.neutral.tone(90));

    schemes.dark.surface = customSchemes.dark?.surface ?? hexFromArgb(theme.palettes.neutral.tone(6));
    schemes.dark.surfaceDim = customSchemes.dark?.surfaceDim ?? hexFromArgb(theme.palettes.neutral.tone(6));
    schemes.dark.surfaceBright = customSchemes.dark?.surfaceBright ?? hexFromArgb(theme.palettes.neutral.tone(24));

    schemes.dark.surfaceContainerLowest =
        customSchemes.dark?.surfaceContainerLowest ?? hexFromArgb(theme.palettes.neutral.tone(4));
    schemes.dark.surfaceContainerLow =
        customSchemes.dark?.surfaceContainerLow ?? hexFromArgb(theme.palettes.neutral.tone(10));
    schemes.dark.surfaceContainer =
        customSchemes.dark?.surfaceContainer ?? hexFromArgb(theme.palettes.neutral.tone(12));
    schemes.dark.surfaceContainerHigh =
        customSchemes.dark?.surfaceContainerHigh ?? hexFromArgb(theme.palettes.neutral.tone(17));
    schemes.dark.surfaceContainerHighest =
        customSchemes.dark?.surfaceContainerHighest ?? hexFromArgb(theme.palettes.neutral.tone(22));
