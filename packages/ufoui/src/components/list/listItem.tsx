...
import { CheckboxIcon, ChevronRightIcon, RadioCheckedIcon, RadioUncheckedIcon } from '../../assets';
 ...
 * @category ListItem
 */
export const ListItem = forwardRef<HTMLDivElement, ListItemProps & ListItemInternalProps>(
    (
        {
            shape,
            touchEffects = ['ripple'],
        ...
        if (type !== 'item') {
            if (type === 'radio') {
                toggleIcon = checked ? (checkedIcon ?? RadioCheckedIcon) : (uncheckedIcon ?? RadioUncheckedIcon);
            } else if (type === 'checkbox') {
                toggleIcon = checked ? (checkedIcon ?? CheckboxIcon) : uncheckedIcon;
            }
        }
        const leadingElement = toggleIcon ?? leading ?? icon ?? (fixedLeading && !horizontal);
        const contentStart = leadingElement && <div className="uui-icon uui-leading">{leadingElement}</div>;
        ...
        const defaultFont: ElementFont = variant === 'modern' ? 'labelLarge' : 'bodyLarge';
        ...
        const contentEnd = (shortcut ?? badge ?? trailingIcon) && (
            <>
                {trailingText}
                {badgeEl}
                {trailingIcon && <div className="uui-icon uui-trailing">{trailingIcon}</div>}
            </>
        );
        ...
        const labelText = label && (
            <div className={`uui-menu-item-label ${getFontClass(appliedLabelFont)}`} style={labelStyle.get()}>
                {label}
            </div>
        );

        const descriptionStyle = ControlStyle();
        if (descriptionColor || (!textColor && variant === 'classic' && color?.startsWith('surface'))) {
            if (descriptionColor) {
        ...
        const content = (
            <div className={contentClasses} style={controlStyle.get()}>
                ...
                <div className={['uui-state', resolvedShape].join(' ')} style={stateStyle.get()} />
            </div>
        );
        ...
        return (
            ...
            <div
                {...props}
                aria-checked={type === 'radio' || type === 'checkbox' ? (checked ?? false) : undefined}
...
type ListItemComponent = ForwardRefExoticComponent<
    ListItemProps & ListItemInternalProps & RefAttributes<HTMLDivElement>
> & {
    [IS_LIST_ITEM]?: true;
    [IS_MENU_ITEM]?: true;
};
/**
 * Marks this component as a ListItem for runtime type guards.
 *
 ...
 * @internal
 */
(ListItem as ListItemComponent)[IS_LIST_ITEM] = true;
(ListItem as ListItemComponent)[IS_MENU_ITEM] = true;
/**
