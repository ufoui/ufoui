...
export const Tabs = ({ defaultValue, children, ...rest }: TabsProps) => {
    ...
    const tabItems: ReactElement<TabProps>[] = Children.toArray(children).filter(isTab);
    ...
    function onKeyDown(e: React.KeyboardEvent<HTMLDivElement>) {
        const currentIndex = tabItems.findIndex(tab => values.includes(tab.props.value));
    ...
    return (
        <SelectionContext.Provider value={contextValue}>
            <BoxBase direction="col" gap={2} {...rest}>
                <div aria-orientation="horizontal" className="uui-tabs-list" onKeyDown={onKeyDown} role="tablist">
                    {buttons}
                </div>
