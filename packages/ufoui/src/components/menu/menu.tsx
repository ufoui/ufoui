...
export const Menu = forwardRef<HTMLDivElement, MenuProps & MenuInternalProps>(
  (
    ...
    const containerElevation: ElementElevation =
      (docked ? dockedElevation : undefined) ??
      elevation ??
      (variant === 'modern' ? 3 : 2);
    ...
    type MenuElement = ReactElement<MenuItemProps & MenuItemInternalProps> &
      ReactElement<DividerProps>;
    ...
    const enabledItemsIndexes = menuItems.reduce<number[]>(
      (acc, item, index) => {
        if (isMenuItem(item)) {
    ...
    function getMenuItemElement(itemIndex: number) {
      ...
      return menuRef.current.querySelector<HTMLDivElement>(
        `[data-menu-index="${String(itemIndex)}"]`,
      );
    }

    function getSubmenuItems(targetIndex: number) {
      const itemChildren = React.Children.toArray(
        menuItems[targetIndex].props.children,
      );
      ...
      const itemSubmenuChildren = React.Children.toArray(
        submenu.props.children,
      );
      return itemSubmenuChildren.filter(
        (el) => isDivider(el) || isMenuElement(el),
      ) as ReactElement[];
    }
    ...
    function hasSubmenu(itemIndex: number) {
      ...
      const itemChildren = React.Children.toArray(
        menuItems[itemIndex].props.children,
    ...
    function _navigateHorizontal(goNext: boolean) {
      ...
      if (goNext) {
        targetIndex =
          enabledItemsIndexes[
            itemPos + 1 < enabledItemsIndexes.length ? itemPos + 1 : 0
          ];
      } else {
        targetIndex =
          itemPos > 0
    ...
    useEffect(() => {
      ...
      const update = () => {
        const resolvedFloatingMode =
          floatingMode ?? (horizontal ? 'dropdown' : 'menu');
    ...
    const clonedChildren = menuItems.map((child, index) => {
      if (isMenuItem(child)) {
        const overriddenProps: MenuItemInternalProps & MenuItemProps = {
          ...
          fixedLeading:
            child.props.fixedLeading ?? (fixedLeading && hasFixedLeading),
          ...
          checkedIcon:
            child.props.checkedIcon ??
            (child.props.type === 'checkbox' ? checkedIcon : radioCheckedIcon),
          uncheckedIcon:
        ...
        const overriddenProps: DividerProps = {
          ...
          inset:
            child.props.inset ?? (variant === 'modern' ? 'middle' : undefined),
        };
    ...
    const animationStyle = {
      ...
      '--uui-duration':
        reverse === 'reverse'
          ? String(openDuration * 0.67) + 'ms'
          : String(openDuration) + 'ms',
    } as React.CSSProperties;

    const resolvedAnimation = resolveAnimation(animation);
    const animationClass =
      resolvedAnimation === 'none' ? '' : motionClassMap[resolvedAnimation];
      ...
      <>
        ...
        {activeItemRef.current && submenuIndex !== -1 && (
          <Menu
            ...
            __groupId={groupId}
            __horizontalDepth={
              horizontal ? 1 : __horizontalDepth && __horizontalDepth + 1
            }
            __level={level + 1}
            __navigateHorizontal={
              horizontal ? _navigateHorizontal : __navigateHorizontal
            }
